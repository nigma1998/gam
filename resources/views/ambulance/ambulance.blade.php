...
@include('inc.message')
    <div>
        ...
        <a href="{{ route('ambulance.ambulance.index') }}">Обновить</a>
...
// за основу взята функция mktime
$sa = $times; // переменная секунда
$xa = $timeh;// переменная час
$ra = $timei + $lafLis->complaint_button;// переменная минуты здесь же мы прибовляем время которая храниться в бд
$ma = date("m", strtotime($lafLis->updated_at));// переменная месяц
$ea = date("d", strtotime($lafLis->updated_at));// переменная день
$ia = date("Y", strtotime($lafLis->updated_at));// переменная год

$endO = mktime($xa,$ra,$sa,$ma,$ea,$ia);
$nw = time(); // текущее время
$secondsRemaining = $endO - $nw; // оставшееся время
...
<form  method="post" action="{{ route('ambulance.buttonpark.update', ['buttonpark'=> $lafLis->id]) }}" onchange="this.form.submit()"  enctype="multipart/form-data">
  @csrf
  @method('put')
  ...
  <button class="btn btn-primary">Собрать экипаж</button>
</form>
@endif
@break
...
$sa = $times; // переменная секунда
$xa = $timeh;// переменная час
$ra = $timei + $lafLis->drink_button;// переменная минуты здесь же мы прибовляем время которая храниться в бд
$ma = date("m", strtotime($lafLis->updated_at));// переменная месяц
$ea = date("d", strtotime($lafLis->updated_at));// переменная день
$ia = date("Y", strtotime($lafLis->updated_at));// переменная год
...
<a href="{{ route('gem.taim.index', ['button'=> $lafLis->id]) }}">Выбрать топливо</a>
@else
<form  method="post" action="{{ route('ambulance.buttonpark.update', ['buttonpark'=> $lafLis->id]) }}" onchange="this.form.submit()"  enctype="multipart/form-data">
  @csrf
  @method('put')
  ...
  <button class="btn btn-primary">Заправиться</button>
</form>
...
@endphp
  @if ($days > -1)
  <div class='blog'>До следующей подсказки
    @if($days > 0)
    {{$days}} д,
    @else
...
<form  method="post" action="{{ route('ambulance.buttonpark.update', ['buttonpark'=> $lafLis->id]) }}"  enctype="multipart/form-data">
  @csrf
  @method('put')
...
@endphp
  ...
  <button class="btn btn-primary">Подсказать маршрут</button>
</form>
...
@endphp
      ...
      <button class="btn btn-primary">Оказать помощь</button>
...
@else
            ...
            <li class="padtop_s first-li">
                    <img class="icon_l" width="48" height="48" src="{{ asset('public/storage/images/skoraj.jpg') }}">
                <div class="row">
                    <div>
                            <span class="patienttitle">Свободный парк</span>
<span class="minor smallfont"></span>                    </div>
                    <div>

    <img width="16" height="16" src="/Themes/images/diagnosis.png"/>
<a href="{{ route('ambulance.ambulance.edit', ['ambulance'=> $lafLis->id]) }}">Принять пациента</a>
                    </div>
                </div>
                <div style="clear: both"></div>
...
<div>
    ...
    <span class="drugtitle">Новый парк</span>
    <div>
                <img width="16" height="16" src="/Themes/images/cart.png"/>
                <form  method="post" action="{{ route('ambulance.ambulance.store') }}" >
                    @csrf
                    <div class="form-group">
    ...
    <ul class="padtop_m">
        ...
        <li class="padtop_m">
            ...
            <a href="">Приемная</a>(в разработке)
        </li>
    </ul>
